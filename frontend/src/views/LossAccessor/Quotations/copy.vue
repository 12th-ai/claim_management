...
export default {
  ...
  // Supporting Documents
  this.form.supportingDocuments.forEach((item) => {
    if (item.file) {
      formData.append('supportingDocuments', item.file); // No indexed keys
    }
    formData.append('supportingDocumentDescriptions[]', item.description); // Separate descriptions
  });

  // Supporting Images
  this.form.supportingImages.forEach((item) => {
    if (item.file) {
      formData.append('supportingImages', item.file); // No indexed keys
    }
    formData.append('supportingImageDescriptions[]', item.description); // Separate descriptions
  });

  // Salvage Rows
  this.form.salvageRows.forEach((row) => {
    formData.append('salvageRows[]', JSON.stringify(row)); // Flatten salvage rows
    if (row.image) {
      formData.append('salvageImages', row.image); // Flat key for salvage images
    }
  });
  ...
  // Logging the FormData to ensure everything is appended correctly
