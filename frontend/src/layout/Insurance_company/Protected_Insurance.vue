...
<script>
import Sidebar from '../../components/Insurance_company/Sidebar.vue';
import TopBar from '../../components/Insurance_company/TopBar.vue';
import axios from 'axios';
import Swal from 'sweetalert2';
import { authService } from '../../services/Company/Auth'; // Import the authService

export default {
  name: 'Protected_company',
  components: {
    Sidebar,
    TopBar,
  },
  ...
  methods: {
    async checkUserProfile() {
      ...
      try {
        ...
        if (!userData) {
          ...
          this.modules = userData.modules || []; // Set modules
        }
      } catch (error) {
    ...
    logout() {
      ...
      Swal.fire({
        ...
        setTimeout(() => {
          this.$router.push('/insurance_company'); // Redirect to the login page
